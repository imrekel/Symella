...
void CAccessPointSettingItem::CreateApNameFromId()
{
	...
	_LIT(KLitNoAccessPoint, "Allways ask");
	if (err)
	{
...
TBool CAccessPointSettingItem::AskIapIdL(TInt32& aIap, TBool aAddAllwaysAsk)
{
	TBuf<200> buf;
	return AskIapIdL(aIap, buf, aAddAllwaysAsk);
}


// !!!! MUST BE REVISED !!!!
TBool CAccessPointSettingItem::AskIapIdL(TInt32& aIap, TDes& aAccesPointName, TBool aAddAllwaysAsk)
    {
    RArray<TUint32> idArray;
	CleanupClosePushL(idArray);
	...
	if (aAddAllwaysAsk)
	{
		...
		namesArray->InsertL(0, _L("Allways ask"));
	}	
